...
namespace FrontierTools
{
    class QuestReader
    {
        ...
        public static bool LoadRankCheck(byte[] src, int index)
        {
            bool check = false;
            string StatTable = null;

            if (Ranks.RankBands.TryGetValue(BitConverter.ToInt32(src, index), out StatTable))
            {
                check = true;
                Console.WriteLine("[RANK][CHECK] (Success) Found Type: {0}     Data Length: {1}     Index: {2}", StatTable, Ranks.RankBands.Count, index);
            }
            else
            {
                check = false;
                Console.WriteLine("[RANK][CHECK] (Fail) Found Type: {0}     Data Length: {1}     Index: {2}", StatTable, Ranks.RankBands.Count, index);
            }
        ...
        {
            string objHex = ReturnObjectiveHex(src, index);
            bool check = false;
            string objType = null;
        ...
        public static string LoadRankInfo(byte[] src, int index)
        {
            string StatTable = null;

            if (Ranks.RankBands.TryGetValue(BitConverter.ToInt32(src, index), out StatTable))
            {
                Console.WriteLine("[RANK][RETURN] (Success) Found Type: {0}     Data Length: {1}     Index: {2}", StatTable, Ranks.RankBands.Count, index);
            }
            else
            {
                Console.WriteLine("[RANK][RETURN] (Fail) Found Type: {0}     Data Length: {1}     Index: {2}", StatTable, Ranks.RankBands.Count, index);
                StatTable = BitConverter.ToInt32(src, index).ToString();
            }

            return StatTable;
        }

        public static string LoadLocationsInfo(byte[] src, int index)
        {
            string location = null;
        ...
        public static string ReturnObjectiveMonsterInfo(byte[] src, int index)
        {
            string monster = null;
        ...
        public static string ReturnObjectiveItemInfo(byte[] src, int index)
        {
            string item = null;
        ...
        public static string ReturnObjectiveTypeInfo(byte[] src, int index)
        {
            string objHex = ReturnObjectiveHex(src, index);
            string objType = null;
